...
Version 0.4 shows coverage where the specification is written. The chain of a specification item opens as a type hierarchy, from the feature it belongs to down to the coverage tags in the source and a summary above each item names the artifact types it still needs and those already covering it. For the whole workspace, a full OpenFastTrace report is now generated from the IDE. Both clients also run the server without a separate Java installation and OpenFastTrace is updated to 4.9.0.
...
* #19: full OpenFastTrace report generated on request, as HTML or plain text in several levels of detail and opened in the IDE
